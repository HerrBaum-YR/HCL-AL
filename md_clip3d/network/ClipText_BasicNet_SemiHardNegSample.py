...
class ClipNet(torch.nn.Module):
   ...
   def forward(self, input_images, input_texts, other_texts):
      ...
      for i in range(input_images.size(0)):
         ...
         logit_per_image_input_text = logit_per_image_all_texts[0, 0]
         logit_per_image_other_texts = logit_per_image_all_texts[0, 1:]

         semi_hard_texts_indices = (logit_per_image_other_texts < logit_per_image_input_text)
         if torch.nonzero(semi_hard_texts_indices).shape[0] < self.topK:
            logit_per_image_semi_hard_texts = logit_per_image_other_texts
         else:
            logit_per_image_semi_hard_texts = torch.masked_select(logit_per_image_other_texts, semi_hard_texts_indices)

         # select the topK highest similarity negative samples
         sorted_logit_per_image_semi_hard_texts, _ = torch.sort(logit_per_image_semi_hard_texts, dim=0, descending=True)
         logit_per_image_topK_texts = sorted_logit_per_image_semi_hard_texts[:self.topK].unsqueeze(0)
